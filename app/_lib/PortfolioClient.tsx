      ...
      <main>
        <header className="grain relative overflow-hidden border-b border-ink-line px-5 py-20 sm:px-8 sm:py-28">
          ...
          <div className="relative mx-auto max-w-4xl">
            <Link
              href="/"
              className="inline-flex items-center gap-1.5 text-sm font-medium text-paper/92 transition-colors hover:text-paper"
            >
            ...
            <p className="mt-5 max-w-lg text-lg text-paper/82">{t.portfolio.subtitle}</p>
          </div>
        </header>
        ...
        {examples.map((ex, i) => (
          ...
          >
            ...
            <div className="flex-1">
              <span className="inline-block rounded-full border border-ink-line px-3 py-1 text-xs font-medium text-paper/84">
                {ex.category}
              </span>
              <h2 className="mt-5 font-serif text-3xl text-paper sm:text-4xl">{ex.name}</h2>
              <p className="mt-3 max-w-md text-paper/82">
              ...
              <div className="mt-6 flex flex-wrap gap-x-8 gap-y-2 border-t border-ink-line pt-5">
                <div>
                  <p className="text-xs uppercase tracking-wider text-paper/74">
                    {t.portfolio.principleLabel}
                  </p>
                  <p className="mt-1 text-sm font-medium text-paper/92">{ex.principle}</p>
                </div>
              </div>

              <ul className="mt-4 flex flex-col gap-1.5">
                {ex.stats.map((s) => (
                  <li key={s} className="flex items-center gap-2 text-sm text-paper/92">
                    <span
